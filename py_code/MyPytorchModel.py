...
from py_code.data.image_folder_dataset import MemoryImageFolderDataset
...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLu(),
            nn.Dropout(p=0.2),
            nn.Linear(100,self.hparamsp["num_classes"])
            )
    ...
    def configure_optimizers(self):
        ...
        def train_model(model, train_loader, val_loader, loss_func, tb_logger, epochs 10, name="default"):
            loss_cutoff = len(train loader) // 10
            optimizer = torch.optim. Adam (model.parameters (), hparams ["'learning rate" ])
            # The scheduler is used to change. the learning rate every few "n" steps.
            scheduler = torch.optim. Ir_scheduler. StepLR(optimizer,
