...
def get_transform():
    ...
    root_path = os.path.join(path, "datasets", 'housing')
    housing_file_path = os.path.join(root_path, "housing_train.csv")
    download_url = 'https://i2dl.vc.in.tum.de/static/data/housing_train.zip'

    train_dataset = CSVDataset(target_column=target_column, root=root_path, download_url=download_url, mode="train")

    # For the data transformations, compute min, max and mean for each feature column. perform the same transformation
